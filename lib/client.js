...
try {
    ...
    console.log("🌍 Connected to the Leader DB")
} catch {
...
let cc = Config.sessionName.replace(/Leader;;;/g, "");
...
const { fetchJson } = require('../lib');	
  ...
  moment.tz.setDefault('Asia/Negombo').locale('id');
  var date = moment.tz('Asia/Negombo').format('DD/MM/YYYY');
  var status = `${textt} \n⏰Time: ${time} \n🚀LEADER 𝚩𝚯𝚻`;
...
app.listen(port, () => console.log(`Leaderr Server listening on port http://localhost:${port}!`));
